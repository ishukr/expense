  ...
  $("#categoryFrom").submit(function (e) {
       ...
       if(data=="")
       {
         $("#span_error").html("Required");   
            ...
            $("#span_error").html("");
 var existingEntries = JSON.parse(localStorage.getItem("roles"));
    if(existingEntries == null) existingEntries = [];
    localStorage.setItem("entry", data);
    // Save roles back to local storage
    existingEntries.push(data);
    localStorage.setItem("roles", JSON.stringify(existingEntries));
...
function callme(param) {
  let display = "";
  let selectCategory = "<option selected disabled>Add to Select roles</option>";
  const Cate_data = JSON.parse(localStorage.getItem("roles"));
...
function remove(index) {
  ...
  localStorage.setItem("roles", JSON.stringify(Cate_data));
   $.getScript("js/org/category.js", function () {
     // //console.log("Added");
   });
}
